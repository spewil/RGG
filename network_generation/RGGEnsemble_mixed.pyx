# add self.radius to the RGGSample class as well!! 

class RGGEnsemble(BaseEnsemble):
	def __init__(self, kappa, n, d, shortcut_prob=0, boundary='s', samples=[], radii=[]):
		self.kappa = kappa
		self.n = n
		self.d = d
	...
	def get_radii(self):
		return [sample.radius for sample in self.samples]
