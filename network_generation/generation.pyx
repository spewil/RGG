...
class RGGEnsemble(BaseEnsemble):
	...
	def compute_radius(self):
		...
		# if sampling from P(d)
		# #Find the value required to get the edges above the required threshold:
		# Edges_Required = (Kappa_Desired*Network_Size)/2.0
		# How far through the list to we have to be?
		# frac = (2*num_edges)/(N*(N-1))
		# coord = int(0.5*frac*(Position_Samples*(Position_Samples-1)))
		# #Get the ' coun'th' element:
