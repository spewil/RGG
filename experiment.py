...
class RGGExperiment(object):
	...
	def list_data(self):
		...
		for ensemble in self.ensembles:
			...
			for sample in ensemble.samples:
				ensemble_nD.append(sample.find_num_unmatched()/float(sample.LCC_size))
				ensemble_deg.append(sample.mean_degree())
	...
	def find_nD_stats_LCC(self):
		...
		for ensemble in self.ensembles:
			sample_nD_list = []	
			for sample in ensemble.samples:
				sample_nD_list.append(float(sample.find_num_unmatched())/sample.LCC_size)
			mean_nD_list.append(np.mean(sample_nD_list))
			std_nD_list.append(stats.sem(sample_nD_list))
			stddev_nD_list.append(np.std(sample_nD_list))
		return mean_nD_list, std_nD_list, stddev_nD_list
...
class GaussianExperiment(object):
	...
	def __init__(self, kappa_range, n, d, shortcut_prob=0, ensembles=[], num_radii=1):
	...
	def add_ensembles(self):
		...
		for kappa in self.kappa_range: 
			# import data 
			GE = ng.GaussianEnsemble(kappa,self.n,self.d, num_radii=self.num_radii)
			try: 
				GE = ng.GaussianEnsemble.from_disk('/Users/spencerw/Documents/DATA/rgg_samples/' + GE.get_param_string()+'.pickle')
			except:
				raise ValueError('Data does not exist for: ' + str(GE.get_param_string()))
			ensembles.append(GE)
		return ensembles	
